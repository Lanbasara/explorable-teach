...
也可以前台跑 `node tutor/server.js`，Ctrl-C 停止——调 `ROLE.md` 时这样更方便看输出。
...
1. **静态服务** — 把教案目录挂到 http 上。课程从此不再受 `file://` 限制
   （Pyodide、sql.js 这类需要 fetch wasm 的组件也就能用了）。
...
## 角色 prompt 在 `ROLE.md` 里

助教的行为定义**不在 `server.js` 的源码里**，而在 `tutor/ROLE.md`。服务启动时读它，
文件缺失或为空就直接报错退出，不会等到你提问时才失败。

这么做是为了**单一事实来源**：`.claude/agents/tutor.md` 那个子 agent 读的是同一个文件。
所以无论你走服务还是走子 agent，拿到的是同一个老师。

想调教助教，改 `ROLE.md`，重启服务即可。

将来的判卷老师只是 `ROLES` 映射表里的另一个条目，指向另一个 `.md` 文件。
...
- 提问 2000 字符上限，选中文本 4000 字符上限，请求体 32KB 上限，单次回答 120 秒超时
